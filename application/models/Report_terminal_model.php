...
//    }
    ...
    function get_all_barcode_report_by_date($start_date,$end_date){
        $terminal_id=$this->session->userdata('user_id');
        $sql="select person_id from person where user_id=?";
        $result = $this->db->query($sql,array($terminal_id));
        if($result==FALSE){
    ...
    function get_terminal_transaction_report($start_date,$end_date){
        $terminal_id=$this->session->userdata('user_id');
        $sql="select person_id from person where user_id=?";
        $result = $this->db->query($sql,array($terminal_id));
        if($result==FALSE){
        ...
        $sql="call transaction_report_by_terminal(?,?,?);";
        $result = $this->db->query($sql,array($person_id,$start_date,$end_date));
        return $result;
    }
...
//    report for card
    ...
    function get_game_full_details_report($series_id,$game_date){
        $terminal_id=$this->session->userdata('user_id');
        $sql="select person_id from person where user_id=?";
        $result = $this->db->query($sql,array($terminal_id));
        if($result==FALSE){
        ...
        $sql="call  game_details_sheet(?,?,?);";
        $result = $this->db->query($sql,array($game_date,$series_id,$person_id));
        return $result;
    }
