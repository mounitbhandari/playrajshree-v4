...
//    }
    ...
    function get_all_barcode_report_by_date($start_date){
        ...
        from (select max(user_id) as user_id,
                    ...
                    , max(play_master.terminal_id) as terminal_id
                    ,max(person.user_id) as user_id
                    , play_details.play_series_id
                    ,max(play_series.mrp) as mrp
                    , max(play_master.draw_master_id) as draw_master_id
                    ,max(play_master.is_claimed) as is_claimed
                    , play_details.row_num
                    , max(play_details.game_value) as game_value
                    , max(draw_master.start_time) as start_time
                    , max(draw_master.end_time) as draw_time
                    , max(draw_master.meridiem) as meridiem
                    ...
                    where date(play_master.ticket_taken_time)=?
                    group by play_details.play_master_id,play_details.play_series_id ,play_details.row_num) as table1
